...
class ScoreCardRisk:
    ...
    def preprocess_data(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        # Impute categorical features with mode and encode them
    ...
    def calculate_correlation_matrix(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        features = [col for col in processed_data.columns if col not in [self.response_var, self.unique_id]]
        corr_matrix = processed_data[features].corr().abs()
        return corr_matrix
